...
func _process(_delta: float) -> void:
	...
	if coopManager.isActive:
		currentPeerCount = coopManager.peers.size()
...
## Returns the port from the input field, or [member CoopManager.DEFAULT_PORT] if empty/invalid.
func GetPort() -> int:
	var portText: String = portInput.text if portInput != null else ""
	if portText.is_valid_int():
		return portText.to_int()
